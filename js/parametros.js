const MAPA_PERMISOS = {
    ...
    catalogos: ['catalogos_productos', 'catalogos_categorias', 'catalogos_proveedores', 'catalogos_sucursales', 'catalogos_clientes']
};
...
window.cargarParametros = async function() {
    ...
    try {
        ...
        // VERIFICACIÓN FORZADA DE PERMISOS (Para arreglar el "Desde Nulo")
        if (adminRole) {
            const { count } = await clienteSupabase.from('permisos_roles').select('*', { count: 'exact', head: true }).eq('id_rol', adminRole.id);
            if (count === 0) {
                console.log("Forzando escritura de permisos base...");
                let permisosFuerza = [];
                
                // Admin: Todo ON menos Admin y Entorno
                ...
                Object.keys(MAPA_PERMISOS).forEach(master => {
                    if (master !== 'admin') {
                        ...
                        MAPA_PERMISOS[master].forEach(hijo => {
                            if (hijo !== 'inventario_recetas') {
                                permisosFuerza.push({ id_rol: colabRole.id, modulo: hijo, puede_ver: true });
                            }
                        });
                    }
                });
        ...
        seleccionarRol(rolInicio.id, rolInicio.nombre, rolInicio.es_predeterminado);
...
function dibujarListaRoles(roles) {
    ...
    listaHtml.innerHTML = roles.map(rol => {
        ...
        const bg = esDueno ? "bg-slate-900 text-white" : "bg-white hover:bg-slate-50 text-slate-700";
        return `
            <li id="li-rol-${rol.id}" onclick="seleccionarRol('${rol.id}', '${rol.nombre}', ${rol.es_predeterminado})" class="px-4 py-3 cursor-pointer flex justify-between items-center transition-colors ${bg}">
...
window.seleccionarRol = async function(idRol, nombreRol) {
    ...
    document.querySelectorAll('#lista-roles-parametros li').forEach(li => {
        if(!li.classList.contains('bg-slate-900')) {
            li.classList.replace('bg-slate-100', 'bg-white');
            li.classList.replace('border-l-4', 'border-l-0');
            li.classList.remove('border-slate-900');
        }
    });
    
    const liActivo = document.getElementById(`li-rol-${idRol}`);
    if(liActivo && !liActivo.classList.contains('bg-slate-900')) {
        liActivo.classList.replace('bg-white', 'bg-slate-100');
        liActivo.classList.add('border-l-4', 'border-slate-900');
    }
    ...
    // Apagar todo visualmente
    toggles.forEach(t => { t.checked = false; t.disabled = false; });
    maestros.forEach(t => { t.checked = false; t.disabled = false; });
    document.querySelectorAll('.permiso-label').forEach(l => {
        l.classList.remove('opacity-40', 'cursor-not-allowed');
    });
...
window.sincronizarMaestroHijo = function(categoriaMaster) {
    ...
    (MAPA_PERMISOS[categoriaMaster] || []).forEach(idHijo => {
        const inputHijo = document.querySelector(`.toggle-permiso[value="${idHijo}"]`);
        if (inputHijo) {
            const label = inputHijo.closest('.permiso-label');
            if (!isON) {
                inputHijo.checked = false; inputHijo.disabled = true;
                if(label) label.classList.add('opacity-40', 'cursor-not-allowed');
            } else {
                if (document.getElementById('nombre-rol-activo').innerText.toLowerCase() !== 'dueño') inputHijo.disabled = false;
